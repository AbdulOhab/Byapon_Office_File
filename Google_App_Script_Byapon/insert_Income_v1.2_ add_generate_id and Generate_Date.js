...
function submitDeta(){
 ...
 if(validEntry() == true){
  ...
  incomeDB.getRange(blankRow, 1).setValue(newEmployeeID); // Employee ID
  incomeDB.getRange(blankRow,2).setValue(myActiveSheet.getRange("C6").getValue()); //date
  incomeDB.getRange(blankRow,3).setValue(myActiveSheet.getRange("C8").getValue()); //Employee Name
  incomeDB.getRange(blankRow,4).setValue(myActiveSheet.getRange("C10").getValue()); //Income Source
  ...
  incomeDB.getRange(blankRow, 6).setValue(formattedDate); // Set the formatted date to the desired cell 

  // Submitted by who
  incomeDB.getRange(blankRow,7).setValue(Session.getActiveUser().getEmail());
